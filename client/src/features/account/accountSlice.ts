...
import { setGameCollection } from "../gameCollection/gameCollectionSlice";
...
export const signInUser = createAsyncThunk<User, any>(
    ...
    async (data, thunkAPI) => {
        try {
            ...
            const { gameCollection, ...user } = userDto;
            if (gameCollection) thunkAPI.dispatch(setGameCollection(gameCollection));
